...
def ul_simulation_target_1(grad_func, xdim, Nx, kbt, c_a, c_b, xinit=None, vinit=None,
    ...
    for i in range(nstep):
        ...
        la = np.sum((x0 - c_a)**2, axis=1) < 0.2**2
        lb = np.sum((x0 - c_b)**2, axis=1) < 0.2**2
        arrival_step = np.logical_or(la, lb)
        x0 = x0[np.logical_not(arrival_step), :]
        v = v[np.logical_not(arrival_step), :]
        arrival[np.logical_not(mask)] = lb
        mask[np.logical_not(mask)] = arrival_step

        # print(x0.shape)

        if np.all(arrival_step):
            print(f'yeah:{i}')
            break
...
if __name__ == '__main__':
    ...
    for i in range(v_sample):
        ...
        arr = np.zeros(x0.shape[0])
        x0 = points
        v = np.random.normal(size=(2)) * np.sqrt(kbt) * 0
        vs[i, :] = v
