...
model_directory = os.path.join(origin_directory, 'designed_muller_result')
...
from model_training import train_resample,pinn_loss,build_rightside,train_resample_qref
...
logging.basicConfig(
    filename=f'designed_muller_result/log/gamma{gamma}_kbt{kbt}.log',        # Specify the log file name
...
logging.info(f'Ref model file: {model_file_ref}')
logging.info(f'Ref config file: {config_file_ref}')
...
subtrain_idx = 0
if subtrain_idx !=0:
    model_file = f'./designed_muller_result/model/gamma{gamma}_kbt{kbt}_subtrain_{subtrain_idx}.pth'
    config_file = f'./designed_muller_result/config/gamma{gamma}_kbt{kbt}_subtrain_{subtrain_idx}.txt'
...
loss_list,b_loss_list,tot_loss_list,pinn_loss_list=train_resample_qref(model=q,
                                          q_ref=q_ref,
                                          data=data,
                                          w=w,
...
# Adjust layout  
plt.savefig(f'./designed_muller_result/fig/loss_gamma{gamma}_kbt{kbt}_subtrain_{subtrain_idx}.png',dpi = 300, bbox_inches='tight')
model_file = f'./designed_muller_result/model/gamma{gamma}_kbt{kbt}_subtrain_{subtrain_idx}.pth'
config_file = f'./designed_muller_result/config/gamma{gamma}_kbt{kbt}_subtrain_{subtrain_idx}.txt'
...
plt.ylabel('x2')
plt.colorbar()
plt.savefig(f'./designed_muller_result/fig/sampling_gamma{gamma}_kbt{kbt}_subtrain_{subtrain_idx}.png',dpi = 300, bbox_inches='tight')
...
# Adjust layout  
plt.savefig(f'./designed_muller_result/fig/loss_gamma{gamma}_kbt{kbt}_total.png',dpi = 300, bbox_inches='tight') 
model_file = f'./designed_muller_result/model/gamma{gamma}_kbt{kbt}.pth'
config_file = f'./designed_muller_result/config/gamma{gamma}_kbt{kbt}.txt'
save_model(q,model_file,config_file)
...
plt.ylabel('x2')
plt.colorbar()
plt.savefig(f'./designed_muller_result/fig/ave_muller_potential_gamma{gamma}_kbt{kbt}.png',dpi = 300, bbox_inches='tight')
...
for i in range(mm):  
    for j in range(nn):  
        idx = j+nn*i
        vvm1 = vs[idx,:]
        simulation = np.loadtxt(f'./designed_muller_result/model/simulation_{kbt}/simulation_kbt{kbt}_gamma{gamma}_{idx}_2.txt')
        ...
        qqq_ref = q_ref(ddd).detach().squeeze().cpu().numpy()
        
        # Create scatter plot  
        sc=axs[i, j].scatter(simulation[:,0],simulation[:,1],c=qqq_ref)  
...
plt.savefig(f'./designed_muller_result/fig/reference_gamma{gamma}_kbt{kbt}.png',dpi = 300, bbox_inches='tight')
...
plt.savefig(f'./designed_muller_result/fig/NN_gamma{gamma}_kbt{kbt}.png',dpi = 300, bbox_inches='tight')
...
for i in range(mm):  
    for j in range(nn):  
        ...
        # Create scatter plot  
        sc=axs[i, j].scatter(simulation[:,0],simulation[:,1],c=np.abs(qqq-qqq_ref))  
        axs[i, j].set_title(f'Scatter Plot {idx+1}')  
...
plt.savefig(f'./designed_muller_result/fig/error_gamma{gamma}_kbt{kbt}.png',dpi = 300, bbox_inches='tight')
...
for i in range(mm):  
    for j in range(nn):  
        ...
        dU1 = potential.gradient(ddd[:,:ndim])
        pinn_l = pinn_loss(q(ddd),ddd,dU1,args)
        pinn_l_ref = pinn_loss(q_ref(ddd),ddd,dU1,args)
        pinn_l = pinn_l.detach().cpu().numpy()
        pinn_l_ref = pinn_l_ref.detach().cpu().numpy()
        
        
        # Create scatter plot  
        sc=axs[i, j].scatter(simulation[:,0],simulation[:,1],c=np.abs(pinn_l-pinn_l_ref))  
        axs[i, j].set_title(f'Scatter Plot {idx+1}')  
        axs[i, j].set_xlabel('x1')  
        axs[i, j].set_ylabel('x2')
        fig.colorbar(sc, ax=axs[i,j])

plt.savefig(f'./designed_muller_result/fig/pinn_gamma{gamma}_kbt{kbt}.png',dpi = 300, bbox_inches='tight')

# In[ ]:


#simulation = np.loadtxt('./model/simulation_kbt.1_gamma10.txt')
q0 = np.loadtxt(f'./designed_muller_result/model/fd_kbt{kbt}.txt')
...
plt.ylabel('x2')
plt.colorbar()
plt.savefig(f'./designed_muller_result/fig/ave_error_gamma{gamma}_kbt{kbt}.png',dpi = 300, bbox_inches='tight')
