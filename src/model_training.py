...
def weight_update_1(model, data, w, dU, batchsize, args, device,
                    xdim, vdim, beta=1, N=10, pinn_weight=0.9, grad_weight=0.05):
    sum = torch.zeros(
        size=(
            data.shape[0],
            1),
    ...
    w[topk_indices, :] *= 2
    w[:] = w / torch.sum(w)
    w[:] = pinn_weight * w + grad_weight * sum + (1 - pinn_weight - grad_weight) * torch.ones(
        size=(data.shape[0], 1), dtype=torch.float32, device=device) / data.shape[0]
    return w
