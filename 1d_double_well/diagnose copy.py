...
for kbt, gamma in zip(kbts, gammas):
    ...
    model_file = f'1d_double_well/model/gamma{gamma}_kbt{kbt}_test.pth'
    config_file = f'1d_double_well/config/gamma{gamma}_kbt{kbt}_test.txt'
    q = load_model(model_file, config_file)
    

    model_file = f'1d_double_well/model/gamma{gamma}_kbt{kbt}_HSS.pth'
    config_file = f'1d_double_well/config/gamma{gamma}_kbt{kbt}_HSS.txt'
    q_HSS = load_model(model_file, config_file)
    
    


    q.to(device=device)
    q_HSS.to(device=device)
    ...
    qqq_HSS = q_HSS(d_grid).squeeze().to('cpu').detach()

    # %%
    vslice = 0 * vmax
    Q = qqq.reshape(X.shape)
    Q_HSS = qqq_HSS.reshape(X.shape)
    ...
    plt.plot(xcal[:], Q_HSS[int((vslice - vmin) / dv), :],
            'g', label='HSS solution')
    ...
    if draw_plot:
        for i in range(mm):
            for j in range(nn):
                ...
                qqq_HSS = qqq_HSS.reshape(X.shape)
                q_NNs.append(qqq_NN)
                e_NNs.append(qqq_NN - fd[:, 2].reshape(X.shape))
                q_HSSs.append(qqq_HSS)
                e_HSSs.append(qqq_HSS - fd[:, 2].reshape(X.shape))
        ...
        save_fig_name_HSS = f'1d_double_well/figures/kbt{kbt}_gamma{gamma}/committor_HSS_slices.png'
        draw_slice(mm,nn,X,V,q_HSSs,save_fig_name_HSS,title_prefix='HSS committor at slice')
        ...
        save_fig_name_res_HSS = f'1d_double_well/figures/kbt{kbt}_gamma{gamma}/res_HSS_slices.png'
        draw_slice(mm,nn,X,V,e_HSSs,save_fig_name_res_HSS,title_prefix='HSS residue at slice')
    ...
    if draw_plot:
        for i in range(mm):
            for j in range(nn):
                ...
                pinn_HSS = pinn_loss(
                    ...
                    dU1,
                    args).squeeze().detach().cpu().numpy()
                pinn_HSS = pinn_HSS.reshape(X.shape)
                pinn_HSSs.append(pinn_HSS)
        ...
        save_fig_name_PINN_HSS = f'1d_double_well/figures/kbt{kbt}_gamma{gamma}/pinn_HSS_slices.png'
        draw_slice(mm,nn,X,V,pinn_HSSs,save_fig_name_PINN_HSS,title_prefix='pinn_loss of HSS at slice')
    ...
    rates_1.append(rate_HSS.item())

    print(f'NN rate: {rate_nn.item()}')
    print(f'Reference rate: {rate_ref}')
    print(f'PINN rate: {rate_HSS.item()}')
